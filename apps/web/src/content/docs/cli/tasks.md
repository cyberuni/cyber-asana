...
| `--tag <gid-or-name[,...]>` | `create` | Tags to apply, each a GID or a tag name resolved in the workspace. With none given, `conventions.default_tags` from the [repo config](/cyber-asana/cli/repo-config/) is used |
